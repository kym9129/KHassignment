...
//		se.incentive(100);
		//((Secretary)employees[0]).incentive(100); 이렇게 한줄로도 가능
//		((Bonus)employees[0]).incentive(100); //Bonus 타입으로도 가능
		
		
//		Sales sa = (Sales)employees[1];
//		sa.incentive(100);
		
		//반복문 돌리려면 Bonus타입 쓰는게 나을 듯
		for(int i = 0; i < employees.length; i++) {
			((Bonus)employees[i]).incentive(100);
		}
		
		//***Employee에서 Bonus를 implement하는게 더 효율적임***
